...
5. 枚举到了一个有效的子域名internal，
   ...
   ********************************************************
   ...
   ![image-20231115120241676](Napper.assets/image-20231115120241676.png)
...
1. 首先研究一下NAPListENER，根据[这篇文章](https://www.elastic.co/security-labs/naplistener-more-bad-dreams-from-the-developers-of-siestagraph)中所述，
   ...
   <img src="Napper.assets/image-20231115131501908.png" alt="image-20231115131501908" style="zoom:50%;" />

   尝试访问/ews/MsExgHealthCheckd/，从响应来看确定目标机器是NAPLISTENER的受害者

   <img src="Napper.assets/image-20231115131609929.png" alt="image-20231115131609929" style="zoom:80%;" />
...
2. 使用Winpeasany.exe检索一下提权向量
   ...
   ![image-20231115163858959](Napper.assets/image-20231115163858959.png)
...
3. 枚举一下目标文件，检索password关键字，可以找到一个oKHzjZw0EGcRxT2cux5K的字符串，但无法利用它作为其他用户的密码切换到其他用户
   ...
   ![image-20231115170531587](Napper.assets/image-20231115170531587.png)
...
5. [elasticsearch](https://book.hacktricks.xyz/network-services-pentesting/9200-pentesting-elasticsearch#banner)手动枚举
   ...
   * Closing connection
   ...
   ![image-20231116091115855](Napper.assets/image-20231116091115855.png)
...
2. 在`C:\Temp\www\internal\content\posts\internal-laps-alpha`下发现一个可执行程序a.exe，将其移到到本地ida中分析
   ...
   ![image-20231116101802840](Napper.assets/image-20231116101802840.png)
...
1. 不是很熟悉golang的反汇编，通过查看大概调用的函数来理解这个可执行文件的功能。
   ...
   ![image-20231116102836535](Napper.assets/image-20231116102836535.png)



2. 调用了main_genKey（），从函数名来看大概是获取什么key

   ![image-20231116103039565](Napper.assets/image-20231116103039565.png)

   双击main_genkey（）查看它的伪代码，`math_rand___Rand__Seed`，google得知这似乎是调用了什么种子

   ![image-20231116103129423](Napper.assets/image-20231116103129423.png)
...
3. 然后是`main_encrypt()`，顾名思义应该是一个加密函数
   ...
   ![image-20231116104117607](Napper.assets/image-20231116104117607.png)
